...
int lrtmp2_script_tag_parse(const uint8_t *data, size_t len, lrtmp2_script_tag_t *tag)
{
    ...
    if (type == AMF0_STRING) {
        size_t slen;
        lrtmf2_amf0_read_string(&buf, tag->name, sizeof(tag->name), &slen);
    } else if (type == AMF0_LONG_STRING) {
        size_t slen;
        lrtmf2_amf0_read_long_string(&buf, tag->name, sizeof(tag->name), &slen);
    } else {
    ...
    if (lrtmf2_amf0_read_type(&buf, &type) == LRTMP2_OK) {
        if (type == AMF0_ECMA_ARRAY) {
            ...
            /* Skip to end — full metadata parsing is optional */
            while (!lrtmf2_amf0_is_object_end(&buf)) {
                char key[256];
                size_t klen;
                lrtmf2_amf0_read_object_key(&buf, key, sizeof(key), &klen);
                lrtmf2_amf0_skip_value(&buf);
            }
            uint8_t end[3];
